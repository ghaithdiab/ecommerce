...
import { getOne,getAll } from "../modules/hundlersFactory.js";
...
export const createCashOrder = asyncHandler(async (req, res, next) => {
  ...
  if (order) {
    const bulkOption = cart.cartItmes.map((item) => ({
      updateOne: {
...
//TODO test this handler on postman 
export const updateOrderToPaid = asyncHandler(async (req, res, next) => {
  const order = await orderModel.findById(req.params.id);
  if (!order) {
    return next(new ApiErrors(`There is no such a order with this id:${req.params.id}`,404));
  }
...
export const updateOrderStatus = asyncHandler(async (req, res, next) => {
  ...
  if(req.body.orderStatus!=="cancelled"){
    if(req.user.role ==='user') return next(new ApiErrors(`You dont have permmistion to this update`,404));
  }
  // update order to paid
  order.orderStatus = req.body.orderStatus ;
  order.deliveredAt =req.body.orderStatus ==='delivered' ? Date.now() : null;
